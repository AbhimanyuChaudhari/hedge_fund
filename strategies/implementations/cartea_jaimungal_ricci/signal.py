...
class CJRicciSignalGenerator:
    """
    Cartea-Jaimungal-Ricci live signal generator.

    Extends CJ signal with:
    - Hawkes process intensity tracking
    - Dynamic spread based on order flow clustering
    - Running inventory penalty logged for monitoring
    """

    def __init__(self, symbol: str, token: int,
                 lot_size: int,
                 instrument_type: str = 'equity_futures',
                 params: CJRicciParameters = None):
        self.symbol          = symbol
        self.token           = str(token)
        self.lot_size        = lot_size
        self.instrument_type = instrument_type
        self.params          = params or CJRicciParameters()
        self.model           = CJRicciModel(self.params)
    ...
    def _get_bars(self, count: int = 60) -> list[dict]:
        ...
        # Record fill events in Hawkes process
        for b in bars:
            if abs(b.get('volume_delta', 0)) > 0:
                self.model.hawkes.add_event(time.time())
    ...
    def _compute_sigma(self, bars: list[dict]) -> float:
        ...
        return float(np.std(np.diff(np.log(prices))) * np.sqrt(len(prices)))
    ...
    def _update_alpha(self, bars: list[dict]):
        ...
        signal  = float(np.clip(imb, -1, 1)) * self.params.alpha_vol

        cj_params   = self.params.to_cj_params()
        from strategies.implementations.cartea_jaimungal.model import CJModel
        cj_tmp      = CJModel(cj_params)
        self._alpha = cj_tmp.alpha_update(self._alpha, signal)
    ...
    def generate(self) -> dict | None:
        ...
        our_bid, our_ask = self.model.optimal_quotes(
            ...
            alpha=self._alpha, sigma=sigma,
            bars=bars
        )

        bid_edge = our_bid - market_bid
        ask_edge = market_ask - our_ask
        min_edge = self.costs.breakeven_spread(mid, 1) / 2
        ...
        return {
            ...
            'sigma':           round(sigma, 6),
            'kappa_eff':       diag['kappa_eff'],
            'hawkes_intensity':diag['hawkes_intensity'],
            'branching_ratio': diag['branching_ratio'],
    ...
    def update_inventory(self, qty: int, side: str):
        if side == 'BUY':
            ...
            self.inventory -= qty
        logger.info(f"{self.symbol} CJRicci inv={self.inventory} "
                    f"α={self._alpha:.4f} κ_eff={self.model._kappa_eff:.4f}")