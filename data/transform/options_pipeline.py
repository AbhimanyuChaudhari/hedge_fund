...
from data.store.s3_client import get_bucket
...
def load_spot_bars(underlying: str, date_str: str) -> pd.DataFrame:
    ...
    if fut_files:
        paths = [f"s3://{f}" for f in sorted(fut_files)[:1]]  # nearest expiry
        try:
...
def run_options_pipeline(underlying: str, date_str: str) -> pd.DataFrame:
    ...
    print(f"Saved → s3://{BUCKET_NAME}/{blob_name}")
