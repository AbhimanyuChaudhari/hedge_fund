...
def list_processed_symbols(client: storage.Client, date: str) -> list[str]:
    ...
    blobs = client.list_blobs(S3_BUCKET, prefix=prefix)
    symbols = []
    for blob in blobs:
...
def save_report_to_gcs(client: storage.Client, date: str, report: dict):
    ...
    log.info(f"Report saved → s3://{S3_BUCKET}/{json_path}")
    log.info(f"Summary saved → s3://{S3_BUCKET}/{txt_path}")
...
def build_summary(report: dict) -> str:
    ...
    lines += [
        "=" * 60,
        f"  GCS log: s3://hedge-fund-494103-marketdata/logs/gap_reports/{report['date']}/",
        "=" * 60,
    ]
