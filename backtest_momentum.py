...
def run_backtest(symbol, dates, lots=20, threshold=0.55,
    ...
    for date in dates:
        ...
        # NEW — momentum signal (NEGATIVE weight from Ridge regression)
        ...
        # From Ridge regression: mom=-0.79, imb=0.11, cancel=-0.11
        # Normalized to sum to 1:
        df['score'] = (0.72 * df['s_momentum'] +   # dominant
                       0.10 * df['s_imb']      +   # small positive
                       0.10 * df['s_cancel']   +   # small positive
                       0.08 * df['s_wmid'])         # small
...
print(f'Testing with momentum-dominant signal (calibrated weights)')
print(f'Dates: {dates}')
print()
print(f'{"thr":>5} {"sl":>4} {"tp":>4} {"hold":>6} {"trades":>7} {"win%":>6} {"total_pnl":>12} {"avg_pnl":>10}')
print('-'*65)
...
for thr,sl,tp,hold in configs:
    r = run_backtest('USDINR26SEPFUT', dates, lots=20,
                    threshold=thr, sl_ticks=sl,
                    tp_ticks=tp, hold_secs=hold)
    wr = r['win_rate']*100
    print(f'{thr:>5.2f} {sl:>4} {tp:>4} {hold:>6} {r["trades"]:>7} '
